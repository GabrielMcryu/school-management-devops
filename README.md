...
```
   ...
   Browser                │                 AKS cluster                   │
  ┌────────┐   HTTPS      │  ┌────────────┐   GitOps    ┌──────────────┐  │
  │ Flutter │────────────▶│  │ ingress-    │◀───sync─────│   Argo CD    │  │
  │  web    │  (Storage   │  │ nginx + TLS │             └──────┬───────┘  │
  └────────┘   static     │  └─────┬───────┘                    │ watches  │
       │       website)   │        │  /api, /health             │  Git     │
       │                  │  ┌─────▼───────┐   ┌────────────┐   │          │
       │   HTTPS API      │  │  web (Django│──▶│  postgres  │   │          │
       └─────────────────▶│  │  + gunicorn)│   │ StatefulSet│   │          │
        nip.io + Let's    │  └─────────────┘   └────────────┘   │          │
        Encrypt cert      │  ┌──────────────────────────────────▼───────┐  │
                          ...
                          └──────────────────────────────────────────────┘
                                         ▲
                          push to main   │  build → test → Trivy → push
                          ┌──────────────┴───────────────┐
                          │  GitHub Actions  →  GHCR      │
                          └──────────────────────────────┘
```
